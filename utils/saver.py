...
class CheckpointSaver(object):
    ...
    def save_checkpoint(self, models, optimizers, epoch, batch_idx, batch_size, dataset_perm, total_step_count,  checkpoint_filename=None):
        """Save checkpoint."""
        timestamp = datetime.datetime.now()

        if checkpoint_filename is not None:
            checkpoint_filename = os.path.abspath(os.path.join(self.save_dir, checkpoint_filename + '.pt'))
        else:
        ...
        mindspore.save_checkpoint(checkpoint, checkpoint_filename)
        return
